...
const fetcher = async (url, pageIndex) => {
  ...
  if (!categoryId) {
    categoryId = await getCatIdFromSlug('5g')
  }
...
const getKey = (pageIndex, previousPageData) => {
  ...
  return [`https://api.pplvs.org/wp-json/wp/v2/posts?page=${pageIndex}&category=5g`, pageIndex] // SWR key
}
...
export default function News() {
  ...
  return (
    <Layout>
      <Head>
        <title>{generatePageTitle('5G / EMF News')}</title>
        <meta name="og:title" content={generatePageTitle('5G / EMF News')} />
      </Head>
